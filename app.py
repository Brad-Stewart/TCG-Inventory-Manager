...
def format_colors_wubrg(colors_list):
    """Convert colors from list to WUBRG order without commas"""
    if not colors_list:
        return ''
    ...
    ordered_colors = [color for color in wubrg_order if color in colors_list]
...
def extract_card_data(data):
    ...
    try:
        ...
        # Calculate mana value from mana cost (don't trust Scryfall's mana_value)
        mana_value = calculate_mana_value(mana_cost)
        
        # Clean up mana cost display (remove brackets)
        mana_cost_display = format_mana_cost_for_display(mana_cost)
        
        # Format colors in WUBRG order without commas
        colors_formatted = format_colors_wubrg(data.get('colors', []))
        ...
        card_info = {
            ...
            'colors': colors_formatted,  # Use WUBRG formatted colors
            'mana_cost': mana_cost_display,  # Use clean display format
            'mana_value': mana_value,  # Use our calculated value
...
def index():
    ...
    cards_query = f'''
        ...
        LIMIT ? OFFSET ?
    '''
    cards = conn.execute(cards_query, where_params + [per_page, offset]).fetchall()
    ...
    colors_raw = conn.execute('SELECT DISTINCT colors FROM cards WHERE user_id = ? AND colors IS NOT NULL AND colors != "" ORDER BY colors', (user_id,)).fetchall()
    ...
    def sort_colors_wubrg(color_tuple):
        color_str = color_tuple[0]
        # Create a sort key based on WUBRG order
        sort_key = []
        wubrg_order = {'W': 0, 'U': 1, 'B': 2, 'R': 3, 'G': 4}
        for char in color_str:
            if char in wubrg_order:
                sort_key.append(wubrg_order[char])
        return (len(sort_key), sort_key)  # Sort by length first, then by WUBRG order
    
    colors = sorted(colors_raw, key=sort_colors_wubrg)
